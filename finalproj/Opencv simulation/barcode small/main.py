        ...
        for (c_x, c_y, r) in circles[0, :]:
            ...
            if lower_dark <= dark_fraction <= upper_dark:
                ...
                if num_blobs == 4:
                    ...
                    if DEBUG:
                        print(
                            f"Valid Hub: Center=({c_x}, {c_y}), Radius={r}, DarkFraction={dark_fraction:.2f}, 4 blobs detected")
                else:
                    reason = f"Blob count: {num_blobs}"
                    failed_candidates.append({"center": (c_x, c_y), "radius": r, "reason": reason})
                    if DEBUG:
                        print(
                            f"Candidate hub at ({c_x}, {c_y}) with radius={r} failed: {num_blobs} blobs detected (DarkFraction={dark_fraction:.2f})")
            else:
